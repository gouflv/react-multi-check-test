...
describe('Test styled helpers - prop', () => {
  ...
  it('should styled function return right value when given props', () => {
    const fn = prop('key', 'default');
    const props = {key: 'foo', name: 'bar'};
    expect(fn(props)).toBe('foo');
  });

  it('should styled function return undefined when key not in props', () => {
    const fn = prop('keeey');
    const props = {key: 'foo'};
    expect(fn(props)).toBeUndefined();
  });

  it('should styled function return default value when key not in props', () => {
